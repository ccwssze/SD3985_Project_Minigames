...
public class Minigame2_Leaderboard_updated : MonoBehaviour
{
    ...
    public void Refresh()
    {
        ...
        var sortPlayerList = (from player in playerList orderby player.GetComponent<JumpingPlayerController_updated>().GetScore() descending select player).ToList();
        ...
        foreach (var player in sortPlayerList)
        {
            ...
            var playerController = player.GetComponent<JumpingPlayerController_updated>();
        ...
        if (JumpingPlayerController_updated.endGame == true)
        {
